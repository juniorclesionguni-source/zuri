...
async function fetchProfile(id: string) {
  const { data } = await supabase!.from('profiles').select('name, genres').eq('id', id).single()
  return data
}
...
export async function signIn(email: string, password: string): Promise<{ user: AuthUser; jwt: string }> {
  ...
  return {
    user: { id: data.user.id, email: data.user.email ?? email, name: p?.name ?? '', genres: p?.genres ?? [] },
...
export async function getProfile(): Promise<AuthUser | null> {
  ...
  return { id: user.id, email: user.email ?? '', name: p?.name ?? '', genres: p?.genres ?? [] }
}
